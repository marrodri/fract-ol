...
void    draw_mand(double width, double height, double val, t_img *st_img, t_manset *st_manset)
{
    ...
    for(st_manset->x = 0; st_manset->x < width; st_manset->x++)
    {
        for(st_manset->y = 0; st_manset->y < height; st_manset->y++)
        {
            ...
            st_manset->y0 = ft_map(st_manset->y, 0, height, -1, 1);
            i = 0;
            st_manset->cx = st_manset->x0;
            st_manset->cy = st_manset->y0;
            st_manset->color = color_iteration(st_manset->color, i, max_i, st_manset->x0, st_manset->y0, st_manset->cx, st_manset->cy, val);
